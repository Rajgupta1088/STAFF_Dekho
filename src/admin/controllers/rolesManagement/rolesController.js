...
const saveRolesPermissions = async (req, res) => {
    ...
    try {
        ...
        const existingUser = await AdminUser.findOne({ email: email }).lean();
        if (existingUser) {
...
const rolesPermissions = (req, res) => {
    ...
    res.render('pages/rolesManagement/manageRoles', { isVendor, admin_type });
}
...
const backendUser = (req, res) => {
    const isVendor = req.session.admin?.isVendor;
    const admin_type = req.session.admin?.admin_type;
    res.render('pages/rolesManagement/backendUser', { isVendor, admin_type });
}
        ...
        if (order && order.length > 0) {
            ...
            switch (parseInt(columnIndex)) {
                ...
                case 2: // Email column
                    ...
                    break;
                default:
                    sort.createdAt = -1; // Default sort by name ascending
                    break;
            }
        } else {
            sort.createdAt = -1; // Default sort by name ascending
