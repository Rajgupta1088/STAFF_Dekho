...
const PaymentDetails = require('../../../api/user/models/paymentModal');
const { sendNotification } = require('../../../api/driver/controllers/notificationController'); // path as needed
const { sendTransactionalSMS } = require('../../../api/user/utils/generateOtp'); // Assuming the common function is located in '../utils/distanceCalculate'
...
const generateOrderId = require('../../../api/user/utils/generateOrderId');
const { generateInvoiceNumber } = require('../../../api/user/utils/generateOtp');
const InitiatePayment = require('../../../api/user/models/paymentModal');

const { packageCalculation } = require('../../../api/user/controllers/packageController');
...
const trackingList = async (req, res) => {
    ...
    try {
        ...
        const orderIdSearch = req.body.orderId;
        ...
        if (searchValue) {
            ...
            if (orderIdSearch) {
                query.orderId = new RegExp(orderIdSearch, 'i');
            }
        ...
        const totalRecords = await Tracking.countDocuments(query);
...
const downloadTrackingCsv = async (req, res) => {
    try {
        ...
        const csvData = trackings.map((tracking, index) => [
            index + 1,
            escapeCsv(tracking.pickupAddress?.replaceAll(',', '-') || ''),
            escapeCsv(tracking.dropAddress?.replaceAll(',', '-') || ''),
            escapeCsv(tracking.orderId || ''),
            escapeCsv(tracking?.userId?.fullName || ''),
            escapeCsv(tracking.totalPayment || 0),
            escapeCsv(getStatusText(tracking.orderStatus)),
            escapeCsv(moment(tracking.deliveryDate).format('YYYY-MM-DD HH:mm:ss')),
            escapeCsv(moment(tracking.createdAt).format('YYYY-MM-DD HH:mm:ss'))
        ]);
        ...
        res.setHeader('Content-Disposition', 'attachment; filename="PTL_Management.csv"');
...
const getDriverWarehouseData = async (req, res) => {
    try {
        ...
        const drivers = await Driver.find({ approvalStatus: 1, serviceType: 1 });
...
const updateOrderStatus = async (req, res) => {
    ...
    try {
        ...
        if (orderStatus == 1) {
            orderAssignDetail.pickupStatus = 1;
            orderAssignDetail.step = 2;
        }
        else if (orderStatus == 2) {
            orderAssignDetail.pickupStatus = 1;
            orderAssignDetail.step = 3;
        }
        else if (orderStatus == 3) {
            orderAssignDetail.pickupStatus = 1;
            orderAssignDetail.step = 4;
        }
        else if (orderStatus == 4) {
            orderAssignDetail.pickupStatus = 1;
            orderAssignDetail.step = 5;
        }
        ...
        let title;
        let message;
        if (orderAssignDetail.status == 1 && orderAssignDetail.assignType == 1) {
            pOrderStatus = 1;
            title = 'Pickup';

        }
        if (orderAssignDetail.status == 2 && orderAssignDetail.assignType == 1) {
            pOrderStatus = 2
            title = 'InTransit';
        }
        if (orderAssignDetail.status == 3 && orderAssignDetail.assignType == 1) {
            pOrderStatus = 2
            title = 'InTransit';
        }
        if (orderAssignDetail.status == 3 && orderAssignDetail.assignType == 2) {
            pOrderStatus = 3
            title = 'Out For Delivery';
        }
        if (orderAssignDetail.status == 4 && orderAssignDetail.assignType == 2) {
            pOrderStatus = 4;
            title = 'Delivered';
        }
        ...
        if (pOrderStatus != 0) {
            const updatedPayment = await PaymentDetails.findOneAndUpdate(
                { _id: orderAssignDetail.packageId },
                { $set: { orderStatus: pOrderStatus } },
                { new: true }
            );
            const ptlNotifyUser = async (screen = 'ptlOrderStatus') => {
                try {
                    await sendNotification({
                        title: title,
                        message: `Your Order ${updatedPayment?.orderId} Has Been ${title}`,
                        recipientId: orderAssignDetail.userId,
                        recipientType: 'user',
                        notificationType: 'order',
                        referenceId: orderAssignDetail?.packageId,
            ...
            const user = await User.findById(orderAssignDetail.userId).select('fullName mobileNumber').lean();


            await ptlNotifyUser("ptlOrderStatus");

            const contactNumber = (updatedPayment.numberForContact && updatedPayment.numberForContact.length > 2)
                ? `91${updatedPayment.numberForContact}`
            ...
            await sendTransactionalSMS(
                ...
                `Successfully ${title} to ${orderAssignDetail?.assignType == 1 ? "Warehouse" : "User Location"}`
            );
        }
...
const addExtraDimension = async (req, res) => {
    try {
        const { extraPackage, extraWeight, extraDimension, packageId } = req.body;

        // Validate input fields
        if (!extraPackage || !extraWeight || !extraDimension || !packageId) {
            return res.status(200).json({
                success: false,
                message: "All fields (extraPackage, extraWeight, extraDimension, packageId) are required"
            });
        }

        // Update the record
        const updateData = await PTL.findByIdAndUpdate(
            packageId,
            {
                $set: {
                    extraPackage,
...
const addManualPackage = async (req, res) => {
    try {
        const {
            pickupLatitude,
            pickupLongitude,
        ...
        if (!userId) return res.json({ success: false, message: "User ID is required in headers." });
        ...
        if (!Array.isArray(packages) || packages.length === 0) {
            return res.json({ success: false, message: "At least one package is required." });
        }
        ...
        for (let i = 0; i < packages.length; i++) {
            ...
            if (
                ...
                return res.json({ success: false, message: `packageName and packageType must be strings (Package ${i + 1}).` });
            }
            ...
            if (
                ...
                return res.json({ success: false, message: `numberOfPackages , totalWeight , height ,width & width must be positive numbers (Package ${i + 1}).` });
            }
        ...
        if (!costResult || costResult.success === false) {
            return res.json({ success: false, message: costResult.message || 'Failed to calculate delivery charges.' });
        }

        const { distance, duration } = costResult;
