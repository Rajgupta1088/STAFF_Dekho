...
const Vendor = require("../../../admin/models/vendorManagement/vendorModel");
...
const { toFixed } = require('../../user/utils/fixedValue');
...
const deleteDriver = async (req, res) => {
    try {
        ...
        const driver = await DriverProfile.findOneAndDelete(
            { _id: driverId }
        );
    ...
    form.parse(req, async (err, fields, files) => {
        ...
        try {
            ...
            if (step === 1) {
                ...
                // const isVendor = getField('isVendor');
                // const vendorId = getField('vendorId');

                // update.isVendor = isVendor || 0;
                ...
                //     const vendor = await Vendor.findById(vendorId).lean(); // drop .lean() if you need instance methods
                //     if (!vendor) {
                //         return res.status(200).json({
                //             success: false,
...
const driverPayoutList = async (req, res) => {
    const driverId = req.header('driverid');
    ...
    try {
        const ftlPaidTrips = await FTL.find({ driverId, transactionStatus: 1, isDriverAmountPayout: 1 }).lean();

        if (!ftlPaidTrips || ftlPaidTrips.length === 0) {
            return res.status(200).json({
                success: false,
                message: 'No payout records found for this driver'
            });
        }
        ...
        // Fetch distances and durations in parallel
        const tripSummaries = await Promise.all(
            ftlPaidTrips.map(async (trip) => {
                const {
                    orderId,
                    totalPayment,
                    driverEarning,
                    isDriverAmountPayout,
                    pickupLatitude,
                    pickupLongitude,
                    dropLatitude,
                    dropLongitude,
                ...
                const distance = result?.distanceInKm || 0;
                const duration = result?.duration || 0;

                totalEarning += Number(driverEarning) || 0;
        ...
        return res.status(200).json({
            success: true,
            message: 'Fetched payout details',
            data: {
                totalTrips: ftlPaidTrips.length,
                totalEarning: toFixed(totalEarning),
                totalDistance: toFixed(totalDistance),
                trips: tripSummaries
            }
        });

    } catch (err) {
        console.error('Error fetching driver payout:', err);
