...
const getTrackingData = async (req, res) => {
    try {
        ...
        const formatTrackingData = (data, source) => ({
            ...
            boxes: source === 'PTL'
                ? (data.packages || []).reduce((sum, el) => sum + (el.numberOfPackages || 0), 0)
                : data.boxes || 0,
            fullDetails: data
        });
        ...
        if (trackingDetails) {
            const responseData = formatTrackingData(trackingDetails, source);
